addLayer("B", {
	...
	buyables: {
        ...
        12: {
            ...
            cost(x) {
                return new Decimal(1000).mul(Decimal.pow(4, x)).mul(Decimal.pow(1.25, Decimal.pow(x, 1.1))).floor()
            },
            display() {
		...
		13: {
            ...
            cost(x) {
                return new Decimal(1e4).mul(Decimal.pow(4, x)).mul(Decimal.pow(1.25, Decimal.pow(x, 1.1))).floor()
            },
            display() {
                return "Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " zeros" + "<br>Bought: " + getBuyableAmount(this.layer, this.id) + "<br>Effect: Boost zero gain by x" + format(buyableEffect(this.layer, this.id))
            },
            ...
            effect(x) {
                let base1 = new Decimal(2)
                let base2 = x
                if(hasUpgrade('B', 21)) base2 = base2.mul(3)
                let base3 = x.add(1)
                if(hasUpgrade('B', 21)) base3 = base3.mul(2)
                let expo = new Decimal(0.6)
                let eff = base1.pow(Decimal.pow(base2, expo)).mul(base3)
                return eff
            },
